...
class GrlStandard:
    def __init__(self, total_iters_each_epoch, total_epochs):
        self.total_iters = total_iters_each_epoch * total_epochs
        self.gamma = 10

    def step(self, accumulated_iter):
        p = accumulated_iter / self.total_iters
        #lambda_ = 0.1 * (2 / (1 + np.exp(-self.gamma * p) ) - 1)
        lambda_ = 0.1
        return lambda_
