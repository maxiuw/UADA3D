...
class IASSD_Backbone(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        #print(batch_dict['centers_origin'].shape)
       ...
       # for i, temp in enumerate(sa_ins_preds):
       #     if torch.is_tensor(temp):
       #         print(f"i: {i}, sa_ins_preds shape: {temp.shape}")

        #print(sa_ins_preds)
        #for i, temp in enumerate(encoder_xyz):
        #    print(f"i: {i}, encoder_xyz shape: {temp.shape}, encoder_features shape: {encoder_features[i].shape}")
        """
        i: 0, encoder_xyz shape: torch.Size([1, 16384, 3]), encoder_features shape: torch.Size([1, 1, 16384])
        i: 1, encoder_xyz shape: torch.Size([1, 8192, 3]), encoder_features shape: torch.Size([1, 64, 8192])
        i: 2, encoder_xyz shape: torch.Size([1, 2048, 3]), encoder_features shape: torch.Size([1, 128, 2048])
        i: 3, encoder_xyz shape: torch.Size([1, 1024, 3]), encoder_features shape: torch.Size([1, 256, 1024])
        i: 4, encoder_xyz shape: torch.Size([1, 512, 3]), encoder_features shape: torch.Size([1, 256, 512])
        i: 5, encoder_xyz shape: torch.Size([1, 512, 3]), encoder_features shape: torch.Size([1, 512, 0])
        i: 6, encoder_xyz shape: torch.Size([1, 512, 3]), encoder_features shape: torch.Size([1, 512, 512])
        """
        ...
        if self.model_cfg.SA_CONFIG.get('SAVE_SAMPLE_LIST',False) and not self.training:
            ...
            result_dir = '/home/matte/sampling_vis'
            print('SAVING SAMPLED PCDS')
            for i in range(batch_size)  :
                # i=0
                # point_saved_path = '/home/yifan/tmp'
                point_saved_path = os.path.join(result_dir, 'puck-source-only')
                os.makedirs(point_saved_path)
                ...
                if '/' in idx: # Kitti_tracking
                    ...
                    sample_xyz = os.path.join(point_saved_path, 'sample_list_' + ('%s' % idx ) )
