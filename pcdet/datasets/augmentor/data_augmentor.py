...
class DataAugmentor(object):
    ...
    #    # Only add random scaling for vehicles
    #    #print('RANDOM OBJECT SCALING:')
    #    #print(data_dict['gt_names'].shape)
    #    vehicle_indices = [i for i, gt_name in enumerate(data_dict['gt_names']) if gt_name == 'Vehicle']
    #    #print(vehicle_indices)
    #    vehicle_boxes = data_dict['gt_boxes'][vehicle_indices]
    #    vehicle_mask = data_dict['gt_boxes_mask'][vehicle_indices]
    #    #print(vehicle_boxes.shape)
    #    #print(vehicle_mask.shape)

    #    points, new_vehicle_boxes = augmentor_utils.scale_pre_object(
    #        vehicle_boxes, data_dict['points'],
    #        gt_boxes_mask=vehicle_mask,
    #        scale_perturb=config['SCALE_UNIFORM_NOISE']
    #    )

    #    data_dict['gt_boxes'][vehicle_indices] = new_vehicle_boxes
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        enable_prob = config.get('ENABLE_PROB', 1.0)  ## default
        rot_range = config['WORLD_ROT_ANGLE']
        if not isinstance(rot_range, list):
            rot_range = [-rot_range, rot_range]
        gt_boxes, points = augmentor_utils.global_rotation(
            data_dict['gt_boxes'], data_dict['points'], rot_range=rot_range, enable_prob=enable_prob
        )
    ...
    def random_world_scaling(self, data_dict=None, config=None):
        ...
        gt_boxes, points = augmentor_utils.global_scaling(
            data_dict['gt_boxes'], data_dict['points'], config['WORLD_SCALE_RANGE'], enable_prob=enable_prob
        )
