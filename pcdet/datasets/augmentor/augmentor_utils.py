...
def random_flip_along_x(gt_boxes, points, enable_prob):
    """
    Args:
...
def random_flip_along_y(gt_boxes, points, enable_prob):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
...
def global_rotation(gt_boxes, points, rot_range, enable_prob):
    """
    Args:
        enable_prob:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
...
def global_scaling(gt_boxes, points, scale_range, enable_prob):
    """
    Args:
...
def scale_pre_object(gt_boxes, points, gt_boxes_mask, scale_perturb, gt_names=None, num_try=50):
    ...
    if isinstance(scale_perturb, dict):
        scale_noises = [np.random.uniform(scale_perturb[gt_names[k]][0], scale_perturb[gt_names[k]][1], size=[1,num_try]) for k in range(num_boxes)]
